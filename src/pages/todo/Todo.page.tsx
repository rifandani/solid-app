...
const TodoPage: Component = () => {
  const { todos, todoForm } = useTodoPageVM();
    ...
    <main class="py-20 px-10 md:px-24 lg:px-40 xl:px-52 flex flex-col items-center justify-center duration-300">
      ...
      <section class="block w-full border rounded p-5 shadow-md border-slate-300">
        <form
          ...
          onSubmit={todoForm.onSubmitTodo}
        >
          <input
            ...
            required
            autofocus
            onKeyUp={todoForm.onKeyUpTodoText}
            value={todoForm.todoText()}
          />
        ...
        >
          {(todo) => (
            <TodoItem
              todo={todo}
              onChangeTodoItemCheckbox={todoForm.onChangeTodoItemCheckbox}
              onDeleteTodoItem={todoForm.onDeleteTodoItem}
            />
          )}
        </For>
      </section>
